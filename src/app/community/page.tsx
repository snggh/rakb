  ...
  return (
    <main>
      <section className="wrap pt-16 pb-14">
        ...
        <p className="max-w-[52ch] text-[13px] text-[var(--dimmer)]">{communityIntro.note}</p>
      </section>
      ...
      <div className="band">
        ...
        >
          {pillars.map((item, i) => (
            <Reveal key={item.title} delay={i * 0.05} className="cell !py-[30px]">
      ...
      <section className="wrap py-14">
        <h2 className="mb-[26px]">The team</h2>
        <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-5">
          {members.map((member, i) => (
            <Reveal key={member.id} delay={i * 0.04}>
              <div>
                <ImageSlot src={member.photo} alt="Photo" ratio="1 / 1" className="mb-3" />
                <div className="text-[15.5px] font-medium tracking-[-0.015em]">{member.name}</div>
                <div className="mono mt-1">{member.role}</div>
                <div className="mt-0.5 text-[12.5px] text-[var(--dimmer)]">{member.kb}</div>
              </div>
            </Reveal>
          ))}
        </div>
        <p className="mt-[18px] text-xs text-[var(--dimmer)]">{membersNote}</p>
      </section>

      <div className="band-top">
        <div className="wrap pt-14 pb-20">
          <h2 className="mb-2.5">Sponsors &amp; vendors</h2>
          <p className="mb-[26px] max-w-[52ch] text-[var(--dim)]">
            Vol. 1 ran on support from shops, artisans, and friends who supplied prizes and equipment.
            Slots for Vol. 2 are open.
          </p>
          <div className="mb-[30px] grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-2.5">
